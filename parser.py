    ...
    for i in range(1, len(blocks) - 1, 2):
        ...
        for line_no in line_nos:
            ...
            if contains_tag_section:
                # 1) Find all compound tags (with slash) or standard tags
                tag_pattern = r'\b[A-Z0-9\-]+/[A-Z0-9\-]+\b|\b[A-Z0-9\-]{2,}-[A-Z0-9\-]{2,}\b'
                for t in re.findall(tag_pattern, tag_block):
                    if cust_po and (t == cust_po or t.startswith(cust_po)):
                        continue
                    tags.append(t)

                # 2) Edge case: combine separate lines like:
                #    TE-P0102
                #    IC0063-NC
                for idx in range(len(lines_clean) - 1):
                    base = lines_clean[idx]
                    nxt  = lines_clean[idx + 1]
                    if (re.fullmatch(r'[A-Z0-9\-]+', base) and
                        re.fullmatch(r'IC\d{2,5}-NC', nxt, re.IGNORECASE)):
                        combo = f"{base}/{nxt}"
                        if combo not in tags:
                            tags.append(combo)

                # 3) Wire-on tags (unchanged)
                ...
                # 4) If qty == 1, keep only the first tag
                if qty.isdigit() and int(qty) == 1 and len(tags) > 1:
                    tags = tags[:1]

            # Universal IC/NC detection (unchanged)
