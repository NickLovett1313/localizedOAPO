        ...
        for line_no in line_nos:
            ...
            if contains_tag_section:
                for idx, line in enumerate(lines_clean):
                    parts = re.findall(r'\b[A-Z0-9\-]{2,}/IC\d{2,5}-NC\b|\b[A-Z0-9\-]{2,}-[A-Z0-9\-]{2,}\b', line)
                    for p in parts:
                        if cust_po and (p == cust_po or p.startswith(cust_po)):
                            continue
                        tags.append(p)

                    # Look ahead for continuation tag pattern like IC0063-NC on next line
                    if re.search(r'\b[A-Z0-9\-]{2,}\b$', line) and (idx + 1 < len(lines_clean)):
                        current = line.strip()
                        next_line = lines_clean[idx + 1].strip()
                        combined = f"{current}/{next_line}"
                        if re.fullmatch(r'[A-Z0-9\-]{2,}/IC\d{2,5}-NC', combined, re.IGNORECASE):
