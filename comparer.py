...
def compare_dates(oa_df, po_df):
    ...
    for (d_oa, d_po), grp in diff.groupby(['Ship Date_OA','Ship Date_PO']):
        starts = sorted(int(n) for n in grp['Line No'] if n.isdigit())
        if not starts:
            continue
        # build identical OA/PO ranges
        oa_range = f"Line {starts[0]}" if len(starts)==1 else f"Lines {starts[0]}–{starts[-1]}"
        po_range = oa_range
        rows.append({
            ...
            '__sort_start':       starts[0]
        })

    df = pd.DataFrame(rows)
    # sort by numeric start of OA Line Range
    df = (
        df
...
def compare_oa_po(po_df, oa_df):
    ...
    # 4) Line-by-line and all other checks (unchanged)...
    # [your existing line-by-line logic here]
