...
def compare_dates(oa_df, po_df):
    # Extract just line number and ship date
    ...
    # Merge and filter where dates differ
    ...
    records = []
    for (oa_date, po_date), group in diff.groupby(['Ship Date_OA','Ship Date_PO']):
        starts = sorted(int(n) for n in group['Line No'] if n.isdigit())
        if not starts:
            continue

        # Numeric-only range: "10–20" or "160"
        start, end = starts[0], starts[-1]
        cell = f"{start}" if start == end else f"{start}–{end}"
    ...
    # Sort by the numeric start of the range, drop helper column
    df = (
        df
